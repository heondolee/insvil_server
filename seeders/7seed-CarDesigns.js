...
module.exports = {
  async up(queryInterface, Sequelize) {
    await queryInterface.sequelize.query('ALTER TABLE CarDesigns AUTO_INCREMENT = 1;');

    let workbook = XLSX.readFile(__dirname + '/../public/stylesheets/7cardesign.xlsx');
    ...
    for (let i = range.s.r + 1; i <= range.e.r; i++) {
      ...
      if (data.length === CHUNK_SIZE) {
        await queryInterface.bulkInsert('CarDesigns', data, {});
        data = [];
      }
    }
  ...
  async down(queryInterface, Sequelize) {
    await queryInterface.bulkDelete('CarDesigns', null, {});
  }
};
